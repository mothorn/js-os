...
//   GET  /api/tube/*      built-in video player (see tube.js)
...
async function handleApi(req, res, url, ctx) {
    ...
    const isTube = url.pathname.startsWith('/api/tube/');
    if (!route && !isTube) {
    ...
    try {
        if (isTube) {
            if (!await tube.handleTube(req, res, url, ctx)) sendJson(res, 404, { error: 'Not found' });
            return true;
        }
        ...
        sendJson(res, status, { error: status === 500 ? 'Internal server error' : err.message }, err instanceof HttpError && err.headers ? err.headers : undefined);
    }
    return true;
}
