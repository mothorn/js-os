...
// Server → client: call-joined {code, peerId, username, session, peers} · peer-joined · peer-left · call-users · signal · error
// Audio itself flows peer-to-peer; the server only relays offers, answers and ICE candidates.
...
function attachCall(wss, ctx) {
    ...
    function onCreate(ws, msg) {
        ...
        rooms.add(room, ws);
        send(ws, { type: 'call-joined', code: room.code, peerId: ws.peerId, username: ws.username, session: ws.session, peers: [] });
    ...
    function onJoin(ws, msg) {
        ...
        send(ws, { type: 'call-joined', code, peerId: ws.peerId, username: ws.username, session: ws.session, peers: existingPeers });
